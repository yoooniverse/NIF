...
## 🚀 **내일 (1월 6일) 작업 계획 - 우선순위별**

### 🔥 **P0 - 긴급 해결 (오전 9:00-11:00)**

- [ ] **Supabase SQL 실행하여 뉴스 표시 즉시 해결**
  - 뉴스 태그 수정: `["international", "politics"]` → `["주식", "부동산"]`
  - 테스트 뉴스 추가 (주식, 부동산, 가상화폐, ETF, 환율 각 1개)
  - `/news/today`에서 뉴스 정상 표시 확인

### 🟡 **P1 - 시스템 개선 (오전 11:00-13:00)**

- [ ] **n8n 워크플로우 RSS 피드 다각화**
  - 현재: 매일경제 경제섹션 1개 RSS (관련 뉴스 부족)
  - 추가: 증권 RSS `https://news.mt.co.kr/newsrss/section.xml?sec_cd=8`
  - 목표: 관심분야별 매칭률 80% 이상 달성
- [ ] **환율 뉴스 RSS 링크 Supabase에 추가**
  - 파이낸셜뉴스 환율 RSS: `https://www.fnnews.com/rss/r20_economy_01.xml`
  - `sources` 테이블에 `source_id='fn_exchange'`로 등록
  - n8n 워크플로우에 환율 뉴스 수집 추가

### 🟢 **P2 - 안정화 및 확장 (오후 14:00-16:00)**
...
- **뉴스 태그 불일치**: DB 뉴스 태그 ↔ 사용자 관심사 매칭 실패
- **RSS 피드 부족**: 투자 관련 뉴스 수집 한계

후) 경제 순환기 분석 프롬프트 고도화 (데이터 품질 향상 및 표현
고도화)### 🎯 **내일 목표**

1. 뉴스 표시 문제 즉시 해결 (SQL 실행)
2. RSS 피드 개선으로 근본 해결
3. 시스템 안정화 및 테스트
