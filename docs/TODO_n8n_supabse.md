# 📅 n8n 뉴스 워크플로우 완료 보고 (2026.01.07)

**최종 상태:** ✅ 뉴스 해설 시스템 완성 및 웹사이트 정상 작동

**전체 진행 상황:** Week 4 / 100% 완료 (경제순환기 지도 + 뉴스 시스템 모두 완료)
**최종 확인:** n8n 뉴스 워크플로우 버그 수정 완료, 웹사이트 뉴스 표시 정상화

---

## ✅ **완료: 맞춤형 뉴스 해설 시스템 구축** (2026.01.07)
...
- [x] **맞춤형 뉴스 해설 시스템 개발** ✅ **(2026.01.07 완료)**
  - [x] 사용자의 관심분야, 레벨, 상황에 맞는 해설 생성 (API 로직 수정: 사용자 Context 우선 매칭)
  - [x] n8n 워크플로우 최적화 및 테스트 완료 ✅
  - [x] 루프 처리 버그 수정 완료 ✅
  - [x] 웹사이트 뉴스 표시 정상화 확인 ✅
...
- [x] **`lib/cycle/determine-status.ts` 파일 생성**
- [x] **경제 지표 기반 점수 계산 함수 구현**
- [x] **Red/Yellow/Green 신호등 색상 결정 로직**
- [x] **입력값 검증 및 에러 핸들링**
...
**증상:** 오늘의 뉴스 페이지(`/news/today`)에 뉴스가 전혀 표시되지 않음

**원인 파악:**

- DB에서 2개 뉴스 가져왔으나 필터링 후 0개로 줄어듦
- 뉴스 태그: `["international", "politics"]` (국제, 정치 뉴스)
- 사용자 관심사: `['부동산', '가상화폐', 'ETF', '주식', '환율']` (투자 관련)
- 태그가 하나도 매칭되지 않아 모든 뉴스가 필터링됨

**해결책 준비:**

- **단기:** Supabase SQL로 뉴스 태그 수정
- **장기:** n8n 워크플로우 RSS 피드 개선 (투자 관련 뉴스 수집)

### **오늘 상황 정리 (완료됨)**

- [x] **오늘의 뉴스 표시 문제 발견 및 분석** ✅

  - 증상: 뉴스가 전혀 표시되지 않음
  - 원인: 뉴스 태그(["international", "politics"]) ↔ 사용자 관심사(['부동산', '가상화폐', 'ETF', '주식', '환율']) 불일치
  - 결과: DB에서 2개 뉴스 가져왔으나 필터링 후 0개로 줄어듦

- [x] **문제 해결책 준비** ✅
  - 단기 해결: Supabase SQL로 뉴스 태그 수정
  - 장기 해결: n8n 워크플로우 RSS 피드 개선
  - 문서 업데이트: TODO_n8n_supabse.md에 상황 정리
...
- [x] 뉴스 필터링 및 온보딩 시스템 디버깅 완료
- [x] **뉴스 해설 시스템 완성** ✅:
  - [x] 뉴스 목록 태그 필터링 (#주식, #사업가 태그 제거)
  - [x] 관심분야 기반 뉴스 필터링 구현
  - [x] **맞춤형 뉴스 해설 시스템 구축 완료** ✅ **(2026.01.07)**
    - [x] n8n 워크플로우 루프 처리 버그 수정 완료
    - [x] 뉴스 데이터 정상 DB 저장 확인
    - [x] 웹사이트 뉴스 표시 정상화 확인
- [x] `determineStatusColor()` 함수 완성
- [x] 신호등 로직 테스트 및 구현
- [x] n8n 워크플로우와 연동 완료 (API 구축)
- [x] 경제 순환기 지도 100% 완성 (실제 데이터 연동 및 n8n 워크플로우 완료)
- [x] **경제순환기 현재 경제 상황 n8n 워크플로우 완성** ✅
  - 데이터 수집 및 저장 완료
  - 페이지 표시 완료
  - 프론트엔드 디자인 완료
- [ ] (v3) 경제 순환기 분석 프롬프트 고도화 (데이터 품질 향상 및 표현 고도화)
...
- [x] **문제 발견 및 분석 완료**
  - 뉴스 태그 불일치로 필터링 실패 (`["international", "politics"]` ↔ `['부동산', '가상화폐', 'ETF', '주식', '환율']`)
- [x] **해결책 준비 완료**
  - 단기: Supabase SQL로 뉴스 태그 수정
  - 장기: n8n 워크플로우 RSS 피드 개선
- [x] **기술 문제 해결**
  - Hydration 에러 수정 (FlightViewBackground dynamic import)
  - 뉴스 데이터 변환 로직 개선
...
- [x] **n8n 워크플로우 버그 수정 완료**
  - 루프 처리 문제 해결
  - 뉴스 데이터 정상 DB 저장 확인
  - `/news/today`, `/news/monthly`에서 뉴스 정상 표시 확인 ✅
...
- [ ] **Supabase RLS 정책 적용** (긴급)

  - 모든 테이블 Row Level Security 활성화
  - 정책 테스트 (Anon vs Authenticated)
  - `users`, `news` 테이블 보안 강화

- [ ] **Lighthouse 성능 점검**
  - 점수 측정 및 개선 (목표: 90점 이상)
  - LCP 최적화
  - CLS 최소화
...
- [ ] **n8n 워크플로우 추가 개선**
  - 50개 뉴스 대량 처리 테스트
  - RSS 피드 추가 (증권, 환율 전문 RSS)
  - 프롬프트 2차 버전 개발 (v3로 이동 가능)

---

## 📊 **현재 시스템 상태** ✅ (2026.01.07)
...
- 사용자 관심사 기반 필터링 ✅
- Supabase DB 연결 ✅
- **n8n 뉴스 워크플로우 완성** ✅ **(2026.01.07)**
  - 뉴스 수집 자동화 완료
  - AI 해설 생성 및 DB 저장 완료
  - 웹사이트 뉴스 표시 정상화
  - 오늘의 뉴스/이달의 뉴스 페이지 정상 작동
- **경제순환기 n8n 워크플로우 완성** ✅
  - 현재 경제 상황 데이터 수집 및 저장
  - 페이지에 데이터 표시 완료
  - 프론트엔드 디자인 완료
...
- ~~뉴스 태그 불일치~~ → ✅ 해결 완료
- ~~RSS 피드 부족~~ → ✅ 현재 구조로 충분히 작동
- ~~n8n 워크플로우 루프 처리 버그~~ → ✅ 해결 완료

### 🚀 **다음 집중 영역**

1. 보안 강화 (RLS 정책 적용)
2. 성능 최적화 (Lighthouse 점검)
3. 전체 QA 및 테스트
...
## 🔴 오늘의 뉴스 표시 문제 (2026.01.05)

### 증상

- 오늘의 뉴스 페이지(`/news/today`)에 뉴스가 전혀 표시되지 않음.
- 터미널 로그: `[API][NEWS_TODAY] News fetched from DB: 2` → `[API][NEWS_TODAY] After JS filtering, count: 0`

### 원인

- **태그 불일치:** 뉴스 데이터의 `interest` 필드와 사용자 관심사가 완전히 다름.
  - 뉴스 태그: `["international", "politics"]` (국제, 정치 관련 뉴스)
  - 사용자 관심사: `['부동산', '가상화폐', 'ETF', '주식', '환율']` (투자 관련 관심사)
- **필터링 로직:** API에서 관심분야 기반으로 뉴스를 필터링하는데, 태그가 하나도 매칭되지 않아 모든 뉴스가 걸러짐.

### 근본 원인

- n8n 워크플로우에서 생성된 뉴스 데이터의 태그가 실제 사용자 관심사와 맞지 않음.
- RSS 피드에서 가져온 뉴스가 일반 경제 뉴스(국제, 정치) 위주로, 투자 관련 뉴스가 부족.
...
```sql
-- 현재 뉴스의 interest 필드를 사용자 관심사와 맞게 변경
UPDATE news_analysis_levels
SET interest = '["주식", "부동산"]'::jsonb
WHERE id = '28b72c53-9786-4493-a225-a7e4425f3c01';
```

2. **더 많은 테스트 뉴스 추가:**

```sql
-- 각 관심분야별로 실제 뉴스 데이터 삽입
INSERT INTO news_analysis_levels (news_id, interest, easy_title, easy_content, normal_title, normal_content) VALUES
('test-news-1', '["주식"]', '삼성전자 실적 호조...', '...', '...', '...'),
('test-news-2', '["부동산"]', '서울 아파트 가격 상승...', '...', '...', '...'),
('test-news-3', '["가상화폐"]', '비트코인 가격 급등...', '...', '...', '...');
```

#### 장기 해결 (워크플로우 개선)

1. **RSS 피드 개선:** 투자 관련 전문 RSS 피드 추가

   - 증권 RSS: `https://news.mt.co.kr/newsrss/section.xml?sec_cd=8`
   - 부동산 RSS: 현재 사용 중인 매일경제 부동산 RSS 유지
   - 환율 RSS: 파이낸셜뉴스 환율 RSS 등 대체 방안 모색

2. **태그 분류 로직 강화:**

   - AI가 뉴스 제목과 내용에 기반하여 더 정확한 관심분야 태그 부여
   - 현재: `["international", "politics"]` → 개선: `["주식", "ETF"]`

3. **품질 검증 추가:**
   - n8n 워크플로우에 태그 검증 단계 추가
   - 생성된 뉴스 데이터의 태그가 실제 관심분야와 일치하는지 확인

### 현재 상태

- ✅ **문제 정확히 파악됨:** 태그 불일치로 인한 필터링 문제
- ✅ **임시 해결책 준비됨:** SQL로 직접 데이터 수정 가능
- 🔄 **장기 해결 진행 중:** RSS 피드 개선 및 워크플로우 최적화
