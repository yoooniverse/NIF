...
- 관심 자산 선택: 부동산, 가상화폐, ETF, 주식, 환율 (멀티 선택)
- 나의 상황 선택: 대출보유, 예적금만함, 달러보유, 사업가, 직장인, 해외여행 (멀티 선택)
- AI 레벨 선택: Lv.1 (초보자), Lv.2 (일반), Lv.3 (전문가) 중 택1
- user_profiles, user_interests, user_contexts 테이블에 저장

**F3. AI 뉴스 파이프라인**

- RSS/News API로 뉴스 수집
- Claude Sonnet 4.5로 분석 (카테고리별 Top 3, 총 15개/일)
- 레벨별 분석 생성 (1개 뉴스 → 3개 레코드)
- news, news_analysis_levels 테이블에 저장

**F4. 메인 대시보드**

- "이달의 뉴스" 섹션
- "오늘의 뉴스" 섹션 (클릭 시 관심분야별 TOP 5 목록 표시)
- 경제 순환기 지도 (간단 버전)
- 뉴스 목록: 제목 + 날짜만 표시 (최신순 정렬)

**F5. 뉴스 상세**

- Block 1: AI 뉴스 해설 (10줄 이내, 쉬운 설명)
- Block 2: 최악의 시나리오 (사용자 상황 기반 개인화)
- Block 3: 행동 가이드 (Action Item)
  - 무료 체험 중(30일): **전체 공개** (블러 없음)
  - 31일차 이후: **블러 처리** → 클릭 시 Paywall
- 출처 링크, 원문 링크

**F6. 구독 시스템**

- Supabase에서 subscriptions 테이블 생성 필요 (아래 스키마 참조)
- 가입 시 subscriptions 테이블에 plan='free', ends_at=now()+30일 자동 생성
- 31일차 active=false → Paywall (콘텐츠 접근 차단 및 결제 유도)
- 토스페이먼츠(Toss Payments) 연동
- 방식: '카드 자동결제(Billing)' 시스템 사용
- 프로세스: 사용자가 결제창에서 카드 등록 → 빌링키(Billing Key) 발급 및 DB 저장
- 가격: 월 9,900원 / 얼리버드 5,900원
- 갱신 로직: 매월 갱신일에 저장된 빌링키로 결제 API 호출하여 구독 연장
...
- 좌측: 순환기별 특징 표 (정적 데이터 - 회복기, 확장기, 둔화기, 침체기)
- 우측: 근거 지표 시각화
  - **신호등 색상**: 규칙 기반 자동 계산 (Red/Yellow/Green)
  - **현재 상황 요약**: AI 작성
  - **역사적 패턴/팩트**: AI 작성
  - **근거 지표**: FRED API에서 수집 (장단기 금리차, 미국 실업률, 원/달러 환율)
