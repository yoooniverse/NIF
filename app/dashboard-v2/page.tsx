...
export default function DashboardV2Page() {
  ...
  const openPanel = (key: PanelKey) => {
    // 핵심 기능 로그: 대시보드 버튼 클릭 → 모달 오픈
    console.info('[DASHBOARD_V2] openPanel:', key);
    setActivePanel(key);
  };

  const closePanel = () => {
    // 핵심 UX 로그: 모달 닫기
    console.info('[DASHBOARD_V2] closePanel');
    setActivePanel(null);
  };

  useEffect(() => {
    console.info('[DASHBOARD_V2] page loaded');
  }, []);
  ...
  useEffect(() => {
    ...
    console.info('[DASHBOARD_V2] user state', {
      userId: user.id,
      onboardingCompleted,
    ...
    if (!onboardingCompleted) {
      console.info('[DASHBOARD_V2] onboarding incomplete -> redirect to /onboarding/interests');
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="mt-5 px-5 flex flex-col flex-1 min-h-0">
              ...
              <div className="mt-3 flex flex-col gap-3 flex-1 min-h-0">
                <button
                  type="button"
                  onClick={() => {
                    // 핵심 기능 로그: 오늘의 뉴스 페이지로 이동
                    console.info('[DASHBOARD_V2] navigate -> /dashboard-v2/today');
                    router.push('/dashboard-v2/today');
                  }}
                ...
                <button
                  type="button"
                  onClick={() => {
                    // 핵심 기능 로그: 이달의 뉴스 페이지로 이동
                    console.info('[DASHBOARD_V2] navigate -> /dashboard-v2/monthly');
                    router.push('/dashboard-v2/monthly');
                  }}
                  className="group w-full flex-1 min-h-[96px] rounded-2xl border border-white/10 bg-white/5 px-5 py-5 text-left hover:bg-white/10 transition"
                >
                  <div className="flex h-full items-center justify-between gap-4">
            ...
            <div className="mt-auto px-5 pb-5">
              <div className="flex items-center justify-between gap-3 text-xs text-white/55">
                <Link
                  href="/dashboard"
                  onClick={() => console.info('[DASHBOARD_V2] click: go legacy dashboard')}
              ...
              <div className="flex items-start justify-between gap-4 px-6 pt-6">
                <div className="leading-tight">
                  ...
                  <div className="mt-1 text-xl font-semibold tracking-tight">
                    {activePanel === 'today' && '오늘의 뉴스'}
                    {activePanel === 'monthly' && '이달의 뉴스'}
                    {activePanel === 'cycle' && '경제 순환기 지도'}
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="px-6 pb-6 pt-4">
                {activePanel === 'today' && (
                  <div className="space-y-3">
                    {MOCK_DATA.todayNews.map((t, idx) => (
                      <div
                        key={`${idx}-${t}`}
                        className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"
                      >
                        <div className="text-xs text-white/50">Headline {idx + 1}</div>
                        <div className="mt-1 text-sm text-white/90 leading-relaxed">{t}</div>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      ...
                      <div className="mt-2 text-sm text-white/90 leading-relaxed">
                        {MOCK_DATA.monthlyBriefing.summary}
                      </div>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="text-xs text-white/50">Highlights</div>
                      <ul className="mt-2 space-y-2 text-sm text-white/85">
                        {MOCK_DATA.monthlyBriefing.highlights.map((h) => (
                ...
                <div className="mt-6 flex items-center justify-between gap-3 text-xs text-white/55">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      console.info('[DASHBOARD_V2] click: modal close (footer)');
                      closePanel();
                    }}
                    className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 hover:bg-white/10 transition"
                  >
